...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.values[(state, action)] = self.values[(state, action)] + self.alpha * (reward + self.discount*self.getValue(nextState) - self.values[(state, action)])
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        return sum([0.0] + [f[ff]*w[ff] for ff in f.keys()])
