...
class getOffensiveActions(Actions):
    ...
    def getFeatures(self, gameState, action):
        ...
        for bd in self.boundary:
            disBoundary = self.agent.getMazeDistance(CurrentPosition, bd)
            if disBoundary < boundaryMin:
                boundaryMin = disBoundary
        features['returned'] = boundaryMin

        # get current carrying score
        ...
        opponentsState = [successor.getAgentState(i) for i in self.agent.getOpponents(successor)]
        opponentsGhost = filter(lambda x: not x.isPacman and x.getPosition() != None, opponentsState)
        opponentsGhost = list(opponentsGhost)
        if len(opponentsGhost) > 0:
            positions = [agent.getPosition() for agent in opponentsGhost]
    ...
    def MCTS(self, depth, gameState, decay):
